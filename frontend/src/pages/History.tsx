    ...
    <Layout headerRight={<HeaderButtons />}>
      <div data-testid='history-page' className='flex flex-col w-full items-center'>
        ...
        <div className='flex flex-col md:flex-row justify-between items-start md:items-center ml-5 mr-5 w-full p-4 gap-3'>
          <h1 data-testid='history-header' className={`${darkMode ? 'text-white' : 'text-black'} font-bold text-4xl flex-shrink-0`}>Transaction History</h1>

          <div className='flex flex-col sm:flex-row w-full md:w-auto gap-4 items-stretch'>
            <div className='relative flex-grow max-w-md'>
              <div className={`absolute inset-y-0 left-0 flex items-center pl-3 ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>
              ...
              <input
                ...
                className={`w-full pl-12 pr-4 py-2 ${darkMode ? 'bg-gray-800 text-white placeholder-gray-400' : 'bg-white/60 text-black placeholder-gray-500'} rounded-full focus:outline-none focus:ring-2 focus:ring-[#FFA294] transition-all duration-200`}
              />
            </div>
            ...
            <button
              ...
              className={`flex flex-row rounded-full pl-6 pr-6 py-2 items-center justify-center ${darkMode ? 'bg-gray-700 text-white' : 'bg-white text-black hover:bg-gray-100'}
              ...
              return filteredData.map((item) => {
                ...
                return (
                  <div key={item._id} className='w-full'>
                    {showDate && (
                      <div className={`font-bold w-full border-b-2 mb-4 pb-2 ${darkMode ? 'text-white border-white' : 'text-black border-b-black/20'}`}>
                        {formatDate(item.transactionDate)}
                      </div>
                    )}

                    <div 
                      className='w-full p-4 bg-white/50 rounded-xl shadow-sm mb-3 hover:shadow-md transition-shadow'
