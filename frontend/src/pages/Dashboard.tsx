    ...
    <Layout headerRight={<HeaderButtons />}>
      ...
      <div className="flex flex-col md:flex-row items-center md:items-center w-full md:px-4 py-8 space-y-6 md:space-y-0 md:space-x-10 min-h-[400px]">
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-center space-y-6 mb-8 md:mb-0 items-center md:items-start text-center md:text-left">
          <div>
            <h2 className={`text-3xl font-bold mb-2 ${darkMode ? "text-white": "text-gray-900"}`}>Send Transactions</h2>
          ...
          <div className="md:flex md:flex-row gap-4 grid grid-cols-2">
            <div className="flex flex-col items-center">
              <button
                ...
                className={`w-18 h-18 sm:w-20 sm:h-20 ${ darkMode ? "bg-gray-700 hover:bg-gray-900" : "bg-[#C6412A] hover:bg-[#A8321E]"} text-white font-small rounded-full cursor-pointer transition duration-300 shadow-md flex items-center justify-center`}
              >
                <IoIosSend className="w-8 h-8" />
              </button>
              <label className={`${darkMode ? 'text-white' : ''} mt-1`}>Send</label>
            </div>
            <div className="flex flex-col items-center">
              <button
              ...
              <label className={`${darkMode ? 'text-white' : ''} mt-1`}>Deposit</label>
            </div>
            <div className="flex flex-col items-center">
              <button
              ...
              <label className={`${darkMode ? 'text-white' : ''} mt-1`}>Withdraw</label>
            </div>
            <div className="flex flex-col items-center">
              <button
              ...
              <label className={`${darkMode ? 'text-white' : ''} mt-1`}>Convert</label>
            </div>
          </div>
        </div>
        ...
        <div className="w-3/4 md:w-1/2 flex flex-col justify-center text-center md:text-left">
          ...
          <h2 data-testid="total-balance-heading" className={`${darkMode ? 'text-white' : ''} text-xl font-extrabold justify-center relative md:left-0 mb-2 md:ml-13`}>Total balance:</h2>

          <div className="flex justify-center md:justify-start md:items-end mb-2 relative md:left-0">
            <p data-testid="wallet-currency" className={`${darkMode ? 'text-white' : ''} text-2xl md:text-3xl font-bold mr-2 md:ml-13`}>
              $
              {userWallets
                .filter(wallet => wallet.walletCurrency === "AUD")
                .reduce((total, wallet) => total + wallet.walletBalance, 0)
                .toLocaleString('en-AU', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </p>
            <p data-testid="aud-currency" className={`${darkMode ? 'text-white' : ''} text-2xl font-semibold`}>AUD</p>
          </div>
      ...
      <div className={`flex flex-col ${darkMode ? 'bg-gray-900' : 'bg-white'} md:flex-row items-center w-full px-4 md:px-10 py-8 space-y-6 md:space-y-0 md:space-x-10 min-h-[400px]`}>
        ...
        <div className="w-full flex flex-col justify-center space-y-6 items-center md:items-end text-center md:text-right">
          <div>
            <h2 className={`text-2xl font-bold mb-2 ${darkMode ? 'text-white' : ''}`}>Send Requests</h2>
          ...
          <button
            ...
            data-testid="send-requests-button"
            className={`w-fit px-6 py-3 ${darkMode? 'bg-gray-700 hover:bg-gray-800' : 'bg-[#C6412A] hover:bg-[#A8312E]'} text-white font-semibold rounded-lg transition duration-300 shadow-md cursor-pointer`}
          >
            Send Requests
          </button>
        </div>
      ...
      <div className="flex flex-col-reverse md:flex-row items-center w-full px-4 md:px-10 py-8 space-y-6 md:space-y-0 md:space-x-10 min-h-[400px]">
        ...
        <img src={'/transaction.png'} className={`${darkMode ? 'bg-white' : ''} w-72 h-72 rounded-full items-end`}></img>
      </div>
    </Layout>
  );
};
