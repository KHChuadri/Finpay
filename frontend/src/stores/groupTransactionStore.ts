...
interface TransactionState {
  ...
  isValidAmountGroupPage: () => boolean;
  isValidRecipientGroupPage: () => boolean;
  setNextPageIfValid: () => void;

  resetGroupData: () => void;
}

const defaultCurrency = { code: 'AUD', countryCode: 'AU', label: 'Australian Dollar', flag: '🇦🇺', localeString: 'en-AU' };

export const useGroupTransactionStore = create<TransactionState>((set, get) => ({
  ...
  isValidAmountGroupPage: () => {
    const { transactionType, sourceCurrencyAmount, destCurrencyAmount } = get();
    if (transactionType == 'TopUp') {
      return sourceCurrencyAmount > 0;
    } else {
      return (sourceCurrencyAmount > 0 && destCurrencyAmount > 0);
    }
  },

  isValidRecipientGroupPage: () => {
      const { transactionType, groupId, recipient } = get();
      if (transactionType === "TopUp") {
  ...
  setNextPageIfValid: () => {
    const { currentPage, setCurrentPage, isValidAmountGroupPage, isValidRecipientGroupPage } = get();
    const validators: Record<number, () => boolean> = {
      1: isValidRecipientGroupPage,
      2: isValidAmountGroupPage,
    };
